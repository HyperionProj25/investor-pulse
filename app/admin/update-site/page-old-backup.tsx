...
type MvpSnapshotState = {
  title: string;
  ctaLabel: string;
  previous: SnapshotTaskState;
  next: SnapshotTaskState;
};
...
const AdminPage = () => {
  ...
  const loadCurrentState = useCallback(async () => {
    try {
      ...
      setFormState({
        ...
        mvpSnapshot: {
          title: shaped.mvpSnapshot.title,
          ctaLabel: shaped.mvpSnapshot.ctaLabel,
          previous: {
            label: shaped.mvpSnapshot.previous.label,
            title: shaped.mvpSnapshot.previous.title,
            description: shaped.mvpSnapshot.previous.description,
            statusLabel: shaped.mvpSnapshot.previous.statusLabel,
          },
          next: {
            label: shaped.mvpSnapshot.next.label,
            title: shaped.mvpSnapshot.next.title,
            description: shaped.mvpSnapshot.next.description,
            statusLabel: shaped.mvpSnapshot.next.statusLabel,
          },
        },
  ...
  const createPayloadObject = useCallback((): QuestionnaireAnswers => ({
    ...
    mvpSnapshot: {
      title: formState.mvpSnapshot.title,
      ctaLabel: formState.mvpSnapshot.ctaLabel,
      previous: {
        label: formState.mvpSnapshot.previous.label,
        title: formState.mvpSnapshot.previous.title,
        description: formState.mvpSnapshot.previous.description,
        statusLabel: formState.mvpSnapshot.previous.statusLabel,
      },
      next: {
        label: formState.mvpSnapshot.next.label,
        title: formState.mvpSnapshot.next.title,
        description: formState.mvpSnapshot.next.description,
        statusLabel: formState.mvpSnapshot.next.statusLabel,
      },
    },
  ...
  const validateForm = useCallback(() => {
    ...
    const requireText = (value: unknown, key: string, label: string) => {
      ...
      if (key.toLowerCase().includes("email")) {
        if (!emailPattern.test(normalized.trim())) {
    ...
    requireText(formState.mvpSnapshot.title, "mvpSnapshot.title", "MVP snapshot title");
    requireText(
      formState.mvpSnapshot.ctaLabel,
      "mvpSnapshot.ctaLabel",
      "MVP snapshot button label"
    );
    (["previous", "next"] as const).forEach((slot) => {
      const task = formState.mvpSnapshot[slot];
      const baseKey = `mvpSnapshot.${slot}`;
      ["label", "title", "description", "statusLabel"].forEach((field) => {
        requireText(
          task[field as keyof SnapshotTaskState],
          `${baseKey}.${field}`,
          `${slot === "previous" ? "Previous" : "Next"} ${humanize(field)}`
        );
      });
    });
  ...
  const setMvpSnapshotField = (key: keyof Pick<MvpSnapshotState, "title" | "ctaLabel">, value: string) => {
    setFormState((prev) => ({
      ...prev,
      mvpSnapshot: { ...prev.mvpSnapshot, [key]: value },
    }));
  };

  const updateMvpTask = (slot: "previous" | "next", field: keyof SnapshotTaskState, value: string) => {
    setFormState((prev) => ({
      ...prev,
      mvpSnapshot: {
        ...prev.mvpSnapshot,
        [slot]: {
          ...prev.mvpSnapshot[slot],
          [field]: value,
        },
      },
    }));
      ...
      >
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <SectionCard
            section="mvpSnapshot"
            title="MVP Snapshot"
            subtitle="Controls the previous/next task card on the investor page"
          ...
          >
            <div className="grid gap-3 md:grid-cols-2">
              {[
                ...
                <label key={key} className="text-xs uppercase tracking-[0.18em] text-[#a3a3a3]">
                  {label}
                  <input
                    className={getInputClasses(`mvpSnapshot.${key}`)}
                    aria-invalid={Boolean(fieldErrors[`mvpSnapshot.${key}`])}
                    value={formState.mvpSnapshot[key as "title" | "ctaLabel"]}
                    onChange={(event) => {
                      clearFieldError(`mvpSnapshot.${key}`);
                      setMvpSnapshotField(key as "title" | "ctaLabel", event.target.value);
                    }}
                  />
                  {renderFieldError(`mvpSnapshot.${key}`)}
                </label>
              ))}
            </div>
            {(["previous", "next"] as const).map((slot) => {
              const task = formState.mvpSnapshot[slot];
              const baseKey = `mvpSnapshot.${slot}`;
              return (
                <div key={slot} className="space-y-3 rounded-xl border border-[#2a2a2a] bg-[#0a0a0a] p-4">
                  <div className="text-xs uppercase tracking-[0.18em] text-[#a3a3a3]">
                    {slot === "previous" ? "Previous task" : "Next task"}
                  </div>
                  <div className="grid gap-3 md:grid-cols-2">
                    {["label", "title"].map((field) => (
                      <label key={field} className="text-xs uppercase tracking-[0.18em] text-[#a3a3a3]">
                        {field}
                        <input
                          className={getInputClasses(`${baseKey}.${field}`)}
                          aria-invalid={Boolean(fieldErrors[`${baseKey}.${field}`])}
                          value={task[field as keyof SnapshotTaskState]}
                          onChange={(event) => {
                            clearFieldError(`${baseKey}.${field}`);
                            updateMvpTask(slot, field as keyof SnapshotTaskState, event.target.value);
                          }}
                        />
                        {renderFieldError(`${baseKey}.${field}`)}
                      </label>
                    ))}
                  </div>
                  <label className="text-xs uppercase tracking-[0.18em] text-[#a3a3a3]">
                    Description
                    <textarea
                      className={getInputClasses(`${baseKey}.description`)}
                      rows={3}
                      aria-invalid={Boolean(fieldErrors[`${baseKey}.description`])}
                      value={task.description}
                      onChange={(event) => {
                        clearFieldError(`${baseKey}.description`);
                        updateMvpTask(slot, "description", event.target.value);
                      }}
                    />
                    {renderFieldError(`${baseKey}.description`)}
                  </label>
                  <label className="text-xs uppercase tracking-[0.18em] text-[#a3a3a3]">
                    Status label
                    <input
                      className={getInputClasses(`${baseKey}.statusLabel`)}
                      aria-invalid={Boolean(fieldErrors[`${baseKey}.statusLabel`])}
                      value={task.statusLabel}
                      onChange={(event) => {
                        clearFieldError(`${baseKey}.statusLabel`);
                        updateMvpTask(slot, "statusLabel", event.target.value);
                      }}
                    />
                    {renderFieldError(`${baseKey}.statusLabel`)}
                  </label>
                </div>
              );
            })}
        ...
        <aside className="space-y-4 rounded-2xl border border-[#1f1f1f] bg-[#0b0b0b] p-4 lg:sticky lg:top-6 lg:h-fit">
          ...
          {previewContent ? (
            <div className="space-y-4 text-sm">
              ...
              <div className={`rounded-xl border bg-[#090909] p-3 ${previewHighlight("mvpSnapshot")}`}>
                <p className="text-xs uppercase tracking-[0.2em] text-[#a3a3a3]">{previewContent.mvpSnapshot.title}</p>
                <div className="mt-2 space-y-2 text-xs text-[#d4d4d4]">
                  <div>
                    <span className="font-semibold">{previewContent.mvpSnapshot.previous.label}:</span> {previewContent.mvpSnapshot.previous.title} ({previewContent.mvpSnapshot.previous.statusLabel})
                  </div>
                  <div>
                    <span className="font-semibold">{previewContent.mvpSnapshot.next.label}:</span> {previewContent.mvpSnapshot.next.title} ({previewContent.mvpSnapshot.next.statusLabel})
                  </div>
                </div>
              </div>
