...
export const QUESTIONNAIRE_PROMPTS: QuestionnairePrompt[] = [
  {
    id: "audience-persona",
    category: "Audience",
...
export const BASELINE_UPDATE: QuestionnaireAnswers = {
  hero: {
    ...
    h1Trail: "progress.",
    mission:
      "Baseline is building the performance data layer for baseball and softball. This page gives investors a single place to see where we are now, what's coming next, and how the round is progressing.",
    descriptor:
      "Send this URL with a query string (e.g., ?investor=amy-chen) and it will welcome them by name with the proof points they're watching.",
  },
  metadata: {
  ...
  tractionNarrative:
    "This section will evolve into a small dashboard of teams, players, events, and key Baseline metrics over time. For now, it's a placeholder while we lock in structure and visuals.",
  investors: [
    {
      slug: "amy-chen",
      name: "Amy Chen",
      ...
      welcomeNote:
        "You asked for proof that we can scale beyond early pilots without losing data quality. Here's the build in flight.",
    ...
    },
    {
      ...
      welcomeNote:
        "You wanted conviction that Baseline can become the default operating system for training centers.",
    ...
    },
    {
      ...
      welcomeNote:
        "You asked how Baseline becomes the connective tissue between tournaments, colleges, and pro scouting.",
    ...
    },
    {
      ...
      welcomeNote:
        "You asked what it looks like when Baseline becomes the nerve center of every elite facility — here's the momentum.",
    ...
    },
    {
      ...
      welcomeNote:
        "Welcome to Baseline's pitch deck. Explore our mission to build the performance data layer for baseball and softball.",
...
export const buildContentFromQuestionnaire = (answers: QuestionnaireAnswers) => {
  const launchTimestamp = Date.parse(answers.metadata.launchTarget);

  return {
    hero: answers.hero,
    metadata: {
      ...answers.metadata,
      launchTimestamp: Number.isNaN(launchTimestamp)
        ? Date.now()
        : launchTimestamp,
      lastUpdatedDisplay: formatDisplayDate(answers.metadata.lastUpdated),
    },
...
export type DerivedContent = ReturnType<typeof buildContentFromQuestionnaire>;
